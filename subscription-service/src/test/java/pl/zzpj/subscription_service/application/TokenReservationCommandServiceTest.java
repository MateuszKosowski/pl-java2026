...
class TokenReservationCommandServiceTest {
  ...
  @Mock private TokenReservationRepository tokenReservationRepository;

  @Mock private TokenReservationPolicy reservationPolicy;
  ...
  void shouldReserveTokensSuccessfully() {
    ...
    TokenReservation reservation =
        new TokenReservation(
            UUID.randomUUID(), userId, TokenOperation.DETECT, 1, fixedInstant.plusSeconds(3600));
  ...
  void shouldConsumeReservationSuccessfully() {
    ...
    TokenReservation reservation =
        new TokenReservation(
            resId, userId, TokenOperation.DETECT, 5, fixedInstant.plusSeconds(3600));
    TokenReservationEntity entity =
        TokenReservationEntity.from(reservation, fixedInstant, "extOp1");

    when(tokenReservationRepository.findById(resId)).thenReturn(Optional.of(entity));
    when(tokenBalanceRepository.findById(userId))
        .thenReturn(Optional.of(TokenBalanceEntity.from(new TokenBalance(userId, 100, 5))));
  ...
  void shouldThrowExceptionWhenConsumingNonOwnedReservation() {
    ...
    TokenReservation reservation =
        new TokenReservation(
            resId, "otherUser", TokenOperation.DETECT, 5, fixedInstant.plusSeconds(3600));
    TokenReservationEntity entity =
        TokenReservationEntity.from(reservation, fixedInstant, "extOp1");

    when(tokenReservationRepository.findById(resId)).thenReturn(Optional.of(entity));

    assertThrows(IllegalArgumentException.class, () -> service.consume(userId, resId));
  }
