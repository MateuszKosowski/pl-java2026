...
4. \texttt{subscription-service} dokonuje rezerwacji tokenów o statusie \texttt{PENDING}. \newline
...
7. \texttt{subscription-service} weryfikuje sesję i jej właściciela, a następnie zmienia status sesji na \texttt{SUCCESS}. \newline
...
\begin{itemize}
    ...
    \item \textbf{ActiveSubscription}: Encja reprezentująca powiązanie użytkownika z konkretnym planem subskrypcyjnym wraz z okresem jego ważności. Posiada metodę \texttt{isExpired()} określającą ważność subskrypcji.
    \item \textbf{TokenBalance}: Encja reprezentująca bilans tokenów zalogowanego użytkownika. Zawiera informację o tokenach dostępnych (\texttt{activeBalance}) oraz zablokowanych na poczet trwających operacji (\texttt{reservedBalance}).
    \item \textbf{TokenReservation}: Reprezentuje rezerwację tokenów dokonaną przez system podczas rozpoczęcia operacji steganograficznej. Posiada unikalne ID, typ operacji, koszt tokenowy oraz status (\texttt{PENDING}, \texttt{CONSUMED}, \texttt{RELEASED}).
...
\begin{figure}[H]
    \centering
    \framebox[\textwidth]{\parbox{0.9\textwidth}{\centering\vspace{3cm} \textbf{[Zrzut ekranu: Panel Logowania i Wyboru Subskrypcji]} \\ \small Przedstawia formularz uwierzytelniania użytkownika oraz listę dostępnych planów subskrypcyjnych (Free, Standard, Pro) wraz z saldem tokenów.\vspace{3cm}}}
    \caption{Placeholder: Ekran wyboru planu subskrypcji i logowania}
    \label{fig:screen_login}
\end{figure}

\begin{figure}[H]
    \centering
    \framebox[\textwidth]{\parbox{0.9\textwidth}{\centering\vspace{3cm} \textbf{[Zrzut ekranu: Kreator Znakowania Obrazów - Embed]} \\ \small Widok przesyłania obrazu PNG, formularz wprowadzania ukrywanej wiadomości, informacja z Capacity Check o limicie znaków oraz wynik klasyfikacji AI (kategoria, ufność).\vspace{3cm}}}
    \caption{Placeholder: Panel osadzania znaku wodnego z klasyfikacją AI}
    \label{fig:screen_embed}
\end{figure}

\begin{figure}[H]
    \centering
    \framebox[\textwidth]{\parbox{0.9\textwidth}{\centering\vspace{3.5cm} \textbf{[Zrzut ekranu: Ekran Detekcji, Ekstrakcji oraz Heatmapy Różnic]} \\ \small Widok wczytanego stemplowanego obrazu, informacje o odczytanej tożsamości właściciela, wyodrębniony tekst steganograficzny oraz wizualizacja heatmapy pixel-diff.\vspace{3.5cm}}}
    \caption{Placeholder: Ekran detekcji znaku wodnego i wizualizacji heatmapy różnic}
