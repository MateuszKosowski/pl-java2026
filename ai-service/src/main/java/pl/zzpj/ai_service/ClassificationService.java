...
public class ClassificationService {
  ...
  /**
   * Loads the MobileNetV2 ONNX model and ImageNet class labels from disk. The synset file is
   * expected to reside in the same directory as the model file.
   */
  @PostConstruct
  ...
  public ClassificationResult classify(MultipartFile file) throws IOException, OrtException {
    ...
    OnnxTensor inputTensor =
        OnnxTensor.createTensor(environment, FloatBuffer.wrap(inputData), shape);
    ...
    int[] topIndices =
        IntStream.range(0, probs.length)
            .boxed()
    ...
    List<ClassificationResult.TopPrediction> top3 =
    ...
    return new ClassificationResult(
        bestLabel, category, bestConfidence, round4(clampedCategoryConfidence), top3);
  }
